...
app.get('/set_cookie',function(request, response){response.cookie('name', 'Preetha Pant').send('cookie set');
});
app.get('/use_cookie',function(request, response){output=`Welcome to the Use Cookie page ${response.send(output);
});
